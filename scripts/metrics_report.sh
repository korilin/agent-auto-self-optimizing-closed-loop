...
END {
  ...
  if (cutover != "") {
    ...
    print "  pre_tasks: " pre_tasks
    print "  pre_tasks_per_day: " safe_avg(pre_tasks, pre_active_days)
    print "  pre_avg_tokens: " safe_avg(pre_tokens, pre_tasks)
    print "  pre_avg_duration_sec: " safe_avg(pre_duration, pre_tasks)
    print "  pre_success_rate: " pct(pre_success, pre_tasks)
    print "  pre_rework_rate: " safe_avg(pre_rework, pre_tasks)
    print "  post_tasks: " post_tasks
    print "  post_tasks_per_day: " safe_avg(post_tasks, post_active_days)
    print "  post_avg_tokens: " safe_avg(post_tokens, post_tasks)
    print "  post_avg_duration_sec: " safe_avg(post_duration, post_tasks)
    print "  post_success_rate: " pct(post_success, post_tasks)
    print "  post_rework_rate: " safe_avg(post_rework, post_tasks)

    if (pre_tasks > 0 && post_tasks > 0) {
